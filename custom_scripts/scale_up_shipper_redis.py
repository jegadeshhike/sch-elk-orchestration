...
parser.add_argument('-e', '--redis_opsworks_layer_id', default='5175c391-cb2a-49bb-be19-4d588d35d430',
                    type=str, help='Opsworks ID of the Redis Layer')
parser.add_argument('-en', '--elk_pipeline_metric_namespace', default='Pipeline1',
                    type=str, help='Custom Cloudwatch metric namespace used for ELK Pipeline')
parser.add_argument('-em', '--elk_pipeline_metric_name', default='ELK1',
                    type=str, help='Custom Cloudwatch metric name used for ELK Pipeline')
...
os.system("echo '* * * * * /usr/bin/aws cloudwatch put-metric-data --metric-name " + 
           elk_pipeline_metric_name + " --namespace " + elk_pipeline_metric_namespace  + 
  ...
  for i in range(0,len(value)) :
    ...
    if shipper_status != 'stopped' :
      raise Exception('Shipper instance with opsworks id ' + shipper_opswork_id + \
                      ' expected status is stopped. It is currently ' + shipper_status + \
...
for key, value in opswork.describe_instances(layer_id=redis_opsworks_layer_id).items() :
  for i in range(0,len(value)) :
    ...
    if redis_status != 'stopped' :
      raise Exception('Redis instance with opsworks id ' + redis_opswork_id + \
                      ' expected status is stopped. It is currently ' + redis_status + \
                      '. Another process could already be running.')
  ...
  for key, value in opswork.describe_instances(instance_ids=instances_to_monitor).items() :
    for i in range(0,len(value)) :
	  ...
	  if status in ['stopped', 'start_failed', 'setup_failed', 'terminating', 'shutting_down', 'terminated'] :	  
	    ...
	    raise Exception( 'One of the shipper and redis instances did not come online successfully.' + \
                             ' Please check Opsworks logs for more details. Shutting them down')
	  elif status == 'online' :
